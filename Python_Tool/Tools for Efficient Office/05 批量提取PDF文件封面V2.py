...
@time: 2025/3/13 16:43:55
@description: 批量提取PDF文件的第一页（封面）并另存为新的PDF文件
@version: 1.0.1

使用说明：
1. 需要安装PyPDF2 3.0.1版本
2. 安装命令：pip install PyPDF2==3.0.1
3. 查看版本：pip show PyPDF2
...
def install_dependencies() -> bool:
    ...
    try:
        ...
        if PyPDF2.__version__ != REQUIRED_PYPDF2_VERSION:
            raise ImportError(f"无法安装指定版本 {REQUIRED_PYPDF2_VERSION}")

        print(f"PyPDF2 {PyPDF2.__version__} 安装成功！")
        return True
    except Exception as e:
        logging.error(f"安装 PyPDF2 失败: {str(e)}")
...
def main():
    ...
    if not install_dependencies():
        print("程序初始化失败，请手动安装指定版本的PyPDF2")
        print(f"执行命令: pip install PyPDF2=={REQUIRED_PYPDF2_VERSION}")
        print("\n按任意键退出...")
        input()
        return
